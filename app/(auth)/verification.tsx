...
const verification = () => {
  ...
  const handleChange1 = (input: string) => {
    // Loại bỏ các ký tự không phải số
    const numericValue = input.replace(/[^0-9]/g, '');
  ...
  const handleChange2 = (input: string) => {
    // Loại bỏ các ký tự không phải số
    const numericValue = input.replace(/[^0-9]/g, '');
  ...
  const handleChange3 = (input: string) => {
    // Loại bỏ các ký tự không phải số
    const numericValue = input.replace(/[^0-9]/g, '');
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={[styles.Pressable, { backgroundColor: '#1A1A1A', marginTop: 200 }]}>
        <Link href={('/(auth)/reset-password')} style={{ fontSize: 16, textAlign: 'center', color: '#FFFFFF', }}>Continue</Link>
      </TouchableOpacity>
    </View>
  )
}
